...
class Signup extends Component{
  ...
  handleSubmit(e) {
      ...
      if(!user.name){alert("Enter the name")}
      if(!user.email){alert("Enter the email")}
      if(!user.phone){alert("Enter the phone")}
      if(!user.password){alert("Enter the password")}      
      if(!user.confirmPassword){alert("Confirm the Password")}
      signupUser(user);
  }
    render(){
        return(
